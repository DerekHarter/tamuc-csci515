...
as input, and what its result should be.  But you do not need to
concern yourself with the messy details.  Likewise, using standard
libraries are examples of a type of reuse.  The functions are written
one time, but reused over and over again in many programs.
