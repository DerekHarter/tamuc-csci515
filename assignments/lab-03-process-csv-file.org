...
In a space or tab separated file, the values separated by commas are
the features, and each row is an individual record.  In the previous
example, this data might represent a database of bank account
