...
\title{Assg 02: Find Average of a Group of Numbers}
...
\item Your program must use only standard C/C++ libraries, headers
  objects and functions.
\item 30+ pts. Your program must implement a sentinel controlled loop.
  The loop must successfully exit on receiving input of the sentinel
  value -9999.
\item 20+ pts. Your program must use integer values for the inputs and
  max/min, but must calculate the average correctly using a floating
  point type.
\item 30+ pts. Your program must correctly keep track of and calculate
  the minimum, maximum and other summary statistics required.
\item 20+ pts. Your program must correctly output the summary statistics,
